-- Create a table for service categories
...
-- Create a join table to link matters and services
...
-- Insert the service categories
INSERT INTO service_categories (name, description) VALUES
  ('Traditional Case Preparation & Litigation Services', 'The foundational set of services central to legal disputes.'),
  ('Technology-Enabled Legal Services', 'These services position the advocate as a manager and strategist in data-intensive matters.'),
  ('Advanced Case & Risk Management Services', 'These services apply formal business and management disciplines to legal matters.'),
  ('Proactive Advisory & Compliance Services', 'These services focus on preventing legal disputes before they arise.'),
  ('Expanded Neutral & Adjudicative Roles', 'These services leverage an advocate''s reputation for impartiality and fairness.'),
  ('Knowledge Transfer & Educational Services', 'These services "productize" an advocate''s specialised knowledge.');
...
CREATE INDEX IF NOT EXISTS idx_matter_services_matter_id ON matter_services(matter_id);
CREATE INDEX IF NOT EXISTS idx_matter_services_service_id ON matter_services(service_id);